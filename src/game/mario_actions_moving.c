...
void align_with_floor(struct MarioState *m) {
    ...
    if ((floor != NULL) && (m->pos[1] < (m->floorHeight + 80.0f))) {
        ...
        if (ABS(m->forwardVel) > FAST_FLOOR_ALIGN) {
            Vec3f floorNormal;
            vec3_copy(floorNormal, floor->normal);
...
void check_ledge_climb_down(struct MarioState *m) {
    ...
    if (m->forwardVel < 10.0f) {
        ...
        if (find_wall_collisions(&wallCols) != 0) {
            f32 floorHeight = find_floor(wallCols.pos[0], wallCols.pos[1], wallCols.pos[2], &floor);
            if ((floor != NULL) && ((wallCols.pos[1] - floorHeight) > 160.0f)) {
                wall = wallCols.walls[wallCols.numWalls - 1];
                s16 wallAngle = SURFACE_YAW(wall);
                if (abs_angle_diff(wallAngle, m->faceAngle[1]) < 0x4000) {
                    m->pos[0] = (wallCols.pos[0] - (20.0f * wall->normal[0]));
                    m->pos[2] = (wallCols.pos[2] - (20.0f * wall->normal[2]));
...
s32 begin_braking_action(struct MarioState *m) {
    ...
    if (m->forwardVel >= 16.0f && m->floor->normal[1] >= COS80) {
