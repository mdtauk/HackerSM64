...
s32 should_get_stuck_in_ground(struct MarioState *m) {
    ...
    if (floor != NULL && (terrainType == TERRAIN_SNOW || terrainType == TERRAIN_SAND)
        ...
        if (!(flags & 0x01) && ((m->peakHeight - m->pos[1]) > 1000.0f) && floor->normal[1] >= COS30) {
            return TRUE;
        }
    }
...
s32 act_jump(struct MarioState *m) {
    ...
    if (m->actionTimer < 1) {
        m->actionTimer++;
        if ((m->input & INPUT_Z_PRESSED) && m->forwardVel > 10.0f) return set_jumping_action(m, ACT_LONG_JUMP, 0);
    }
#endif
...
s32 act_butt_slide_air(struct MarioState *m) {
    ...
    switch (perform_air_step(m, 0)) {
        case AIR_STEP_LANDED:
            if (m->actionState == 0 && m->vel[1] < 0.0f && m->floor->normal[1] >= COS10) {
